        ...
        <form class="space-y-6">
            ...
            <div class="p-4 border border-gray-200 rounded-lg">
                ...
                <div v-if="distance" class="mt-2 text-sm text-right text-gray-600">
                    {{ distance }} km × {{ tauxFraisStore.tauxFrais.KM.taux }}€ = <span class="font-medium text-indigo-600">{{ montantKm.toFixed(2) }}€</span>
                </div>
            </div>
            ...
            <div class="p-4 border border-gray-200 rounded-lg">
                ...
                <div v-if="nbRepas" class="mt-2 text-sm text-right text-gray-600">
                    {{ nbRepas }} repas × {{ tauxFraisStore.tauxFrais.REP.taux }}€ = <span class="font-medium text-indigo-600">{{ montantRepas.toFixed(2) }}€</span>
                </div>
            </div>
            ...
            <div class="p-4 border border-gray-200 rounded-lg">
                ...
                <div v-if="nbHebergements" class="mt-2 text-sm text-right text-gray-600">
                    {{ nbHebergements }} nuits × {{ tauxFraisStore.tauxFrais.NUI.taux }}€ = <span class="font-medium text-indigo-600">{{ montantHebergement.toFixed(2) }}€</span>
                </div>
            </div>
            ...
            <!-- Résumé des frais -->
            <div v-if="montantTotal > 0" class="bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                <div class="flex items-center mb-3">
                    ...
                    <span class="font-medium text-gray-700">Total à rembourser</span>
                </div>
                <div class="space-y-2">
                    <div v-if="distance" class="flex justify-between text-sm">
                        <span class="text-gray-600">Kilométrage ({{ distance }} km × {{ tauxFraisStore.tauxFrais.KM.taux }}€)</span>
                        <span class="font-medium">{{ montantKm.toFixed(2) }}€</span>
                    </div>
                    <div v-if="nbRepas" class="flex justify-between text-sm">
                        <span class="text-gray-600">Repas ({{ nbRepas }} × {{ tauxFraisStore.tauxFrais.REP.taux }}€)</span>
                        <span class="font-medium">{{ montantRepas.toFixed(2) }}€</span>
                    </div>
                    <div v-if="nbHebergements" class="flex justify-between text-sm">
                        <span class="text-gray-600">Hébergement ({{ nbHebergements }} × {{ tauxFraisStore.tauxFrais.NUI.taux }}€)</span>
                    ...
                    <div class="pt-2 mt-2 border-t border-indigo-200 flex justify-between items-center">
                        ...
                        <span class="text-xl font-bold text-indigo-600">{{ montantTotal.toFixed(2) }}€</span>
                    </div>
                </div>
            </div>
