...
const tauxFormatted = computed(() => {
    return tauxFraisStore.tauxFrais.NUI.taux.toFixed(2);
});

const montantBase = computed(() => {
    if (!fraisData.value.nbNuits) return 0;
    return parseFloat(fraisData.value.nbNuits) * parseFloat(tauxFraisStore.tauxFrais.NUI.taux);
});

const montantTotal = computed(() => {
    let total = montantBase.value;

    if (fraisData.value.fraisSejour && fraisData.value.taxeSejour) {
        total += parseFloat(fraisData.value.nbNuits) * parseFloat(fraisData.value.taxeSejour);
    }
...
const champsRemplis = computed(() => {
    return fraisData.value.dateArrivee &&
        fraisData.value.nbNuits &&
        fraisData.value.description;
});
...
const submitFormHebergement = async () => {
    ...
    try {
        const response = await fetch("http://51.83.76.210/gsb/backend/addFormHebergement.php", {
            ...
            body: JSON.stringify({
                dateArrivee: fraisData.value.dateArrivee,
                description: fraisData.value.description,
        ...
        setTimeout(() => {
            NotifSuccess(`Frais d'hébergement ajoutés avec succès`);
    ...
    <form class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-2">
            ...
            <div class="relative">
                ...
                <input type="date" v-model="fraisData.dateArrivee"
        ...
        <div class="space-y-2">
            ...
            <input type="number" min="1" placeholder="Ex: 1" v-model="fraisData.nbNuits"
        ...
        <div class="space-y-4">
            ...
            <div v-if="fraisData.fraisSejour" class="pl-7">
                ...
                <div class="relative rounded-lg shadow-sm">
                    <input type="number" step="0.01" v-model="fraisData.taxeSejour"
                        class="w-full p-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        placeholder="Ex: 2.50">
        ...
        <div class="md:col-span-2 space-y-2">
            ...
            <textarea v-model="fraisData.description" rows="3"
                class="w-full p-2.5 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                placeholder="description du déplacement (visite client, salon professionnel, formation, etc.)"></textarea>
        </div>

        <div v-if="fraisData.nbNuits" class="md:col-span-2 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div class="space-y-2">
                <div class="flex justify-between text-sm">
                    <span>Hébergement ({{ fraisData.nbNuits }} nuit{{ fraisData.nbNuits > 1 ? 's' : '' }} × {{
                        tauxFormatted }}€)</span>
                    <span>{{ montantBase.toFixed(2) }}€</span>
                </div>

                <div v-if="fraisData.fraisSejour && fraisData.taxeSejour" class="flex justify-between text-sm">
                    <span>Taxe de séjour ({{ fraisData.nbNuits }} × {{
                        parseFloat(fraisData.taxeSejour).toFixed(2) }}€)</span>
                    <span>{{ (parseFloat(fraisData.nbNuits) * parseFloat(fraisData.taxeSejour)).toFixed(2)
                        }}€</span>
                </div>
        ...
        <div class="md:col-span-2 flex justify-end space-x-4 pt-4">
            ...
            <button type="submit" @click.prevent="submitFormHebergement"
